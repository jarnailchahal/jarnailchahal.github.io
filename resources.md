---
layout: page
title: Resources
subtitle: Some great online materials, notes and resources!
---

These are  some of my favourite resources I refer to while learning stuff.

###  Python

* [Python Basics](https://chrisalbon.com/#python)
* [Python Crash Course](https://www.grahamwheeler.com/posts/python-crash-course.html)
* [Data Visualization - Plotly Python Open Source Graphing Library](https://plotly.com/python/)
* [Dash - Python framework for building web applications](https://dash.plotly.com)
* [Awesome Python - Curated list of  Python frameworks and libraries](https://github.com/vinta/awesome-python)

###  R

* [R Bootcamp](https://www.jaredknowles.com/r-bootcamp/)
* [Simple Linear Regression in R](http://www.sthda.com/english/articles/40-regression-analysis/167-simple-linear-regression-in-r)
* [Advanced R](http://adv-r.had.co.nz/)

###  Machine Learning

* [Hands on Machine Learning with R](https://bradleyboehmke.github.io/HOML/)
